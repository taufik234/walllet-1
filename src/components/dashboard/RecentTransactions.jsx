...
import { ArrowRight, Wallet, ShoppingBag, Utensils, Car, Film, Gift, TrendingUp, MoreHorizontal, Heart, GraduationCap, Receipt, Briefcase } from 'lucide-react';
                    ...
                    return (
                        <div key={trx.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-xl flex items-center gap-4 hover:border-slate-300 dark:hover:border-slate-700 transition-colors shadow-sm dark:shadow-none">
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-slate-900 dark:text-white font-medium capitalize truncate">{trx.note || categoryName}</p>
                                <p className="text-xs text-slate-500">{formatDate(trx.date)} • <span className="capitalize">{categoryName}</span></p>
                            </div>
