...
export default function TransactionDetailModal({ isOpen, onClose, transaction }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-slate-900 w-full max-w-sm rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 relative">
                {/* Receipt Pattern Top */}
                ...
                <div className="p-6 relative">
                    ...
                    <div className="flex flex-col items-center pt-4 pb-8">
                        <div className={`w-16 h-16 rounded-3xl flex items-center justify-center mb-4 ${isIncome ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'
                            }`}>
                        ...
                        <h2 className={`text-3xl font-bold ${isIncome ? 'text-white' : 'text-white'
                            }`}>
                    ...
                    <div className="bg-slate-950/50 rounded-2xl p-5 space-y-4 border border-slate-800/50">
                        ...
                        {transaction.note && (
                            <div className="pt-1">
                                ...
                                <p className="text-slate-300 text-sm italic">"{transaction.note}"</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 mt-6">
                        <button
                            onClick={handleEdit}
                            className="flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-indigo-400 font-medium py-3 rounded-xl transition-colors"
                        >
                            <Pencil className="w-4 h-4" /> Edit
                        </button>
                        <button
                            onClick={handleDelete}
                            className="flex items-center justify-center gap-2 bg-slate-800 hover:bg-rose-900/30 text-rose-400 font-medium py-3 rounded-xl transition-colors"
                        >
