...
export default function WalletFilter({ value, onChange }) {
    ...
    const options = [
        ...
        ...wallets.map(w => ({
            ...w,
            // Map icon based on wallet TYPE, not ID (since ID is UUID)
            icon: w.type === 'cash' ? Wallet : w.type === 'bank' ? CreditCard : Smartphone
        }))
    ];

    return (
        <div className="flex flex-wrap gap-2 bg-slate-100 dark:bg-slate-900/50 p-1 rounded-2xl border border-slate-200 dark:border-slate-800/50">
            {options.map((opt) => {
                const Icon = opt.icon;
